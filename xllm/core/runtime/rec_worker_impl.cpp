...
void RecWorkerImpl::LlmRecPureDevicePipeline::prepare_work_before_execute(
  ...
  if (is_pure_device_mode()) {
    ...
    // Read from ForwardInput fields (input parameters)
    ...
    mip.llm_rec_pure_device_params.full_k_caches.clear();
    mip.llm_rec_pure_device_params.full_v_caches.clear();
    mip.llm_rec_pure_device_params.full_k_caches.reserve(num_layers);
    mip.llm_rec_pure_device_params.full_v_caches.reserve(num_layers);
    mip.llm_rec_pure_device_params.unshared_k_caches.clear();
    mip.llm_rec_pure_device_params.unshared_v_caches.clear();
    mip.llm_rec_pure_device_params.unshared_k_caches.reserve(num_layers);
    mip.llm_rec_pure_device_params.unshared_v_caches.reserve(num_layers);
    for (int32_t layer_id = 0; layer_id < num_layers; ++layer_id) {
      ...
      mip.llm_rec_pure_device_params.full_k_caches.emplace_back(
          target_layer_full_k_cache);
      mip.llm_rec_pure_device_params.full_v_caches.emplace_back(
          target_layer_full_v_cache);
      mip.llm_rec_pure_device_params.unshared_k_caches.emplace_back(
          target_layer_unshared_k_cache);
      mip.llm_rec_pure_device_params.unshared_v_caches.emplace_back(
          target_layer_unshared_v_cache);
    }
    {
      const auto& dec_pos = processed_inputs.decode_positions_vec;
      mip.llm_rec_pure_device_params.decode_positions_tensor_list.clear();
      if (!dec_pos.empty() && beam_width > 0 && total_round > 1) {
        ...
        for (int j = 0; j < total_round - 1; ++j) {
          ...
          mip.llm_rec_pure_device_params.decode_positions_tensor_list.push_back(
    ...
    {
      mip.llm_rec_pure_device_params.naive_block_table =
          torch::arange(batch_size, int_options).unsqueeze(1);
    }
    // set LLM Rec pure device metadata
    mip.llm_rec_pure_device_params.beam_width = beam_width;
    mip.llm_rec_pure_device_params.current_round = current_round;
    mip.llm_rec_pure_device_params.total_round = total_round;
  }
#endif
}
...
std::optional<ForwardOutput> RecWorkerImpl::LlmRecPureDevicePipeline::step(
  ...
  // Read from ForwardInput fields (input parameters), not from
  // llm_rec_pure_device_params llm_rec_pure_device_params is for storage,
  // ForwardInput fields are the source of truth
  ...
  mutable_input.input_params.llm_rec_pure_device_params.beam_width = beam_width;
  ...
  mutable_input.input_params.llm_rec_pure_device_params.preallocated_output =
  ...
  for (int32_t round = 0; round < total_rounds; ++round) {
    ...
    mutable_input.input_params.llm_rec_pure_device_params.current_round =
        round - 1;
    ...
    if (round == 0) {
      mutable_input.input_params.llm_rec_pure_device_params
          .preallocated_output = torch::zeros(
    ...
    if (VLOG_IS_ON(10)) {
      auto sequence_group_cpu = beam_tensors.sequence_group.cpu();
      auto acc_logprob_cpu =
          beam_tensors.acc_logprob.view({batch_size, beam_width}).cpu();

      auto seq_group_accessor = sequence_group_cpu.accessor<int32_t, 3>();
      auto logprob_accessor = acc_logprob_cpu.accessor<float, 2>();

      for (int i = 0; i < batch_size; i++) {
        for (int j = 0; j < beam_width; j++) {
          std::string str{};
          for (int k = 0; k < seq_group_accessor.size(2); k++) {
            if (k > 0) {
              str += ", ";
            }
            str += std::to_string(seq_group_accessor[i][j][k]);
          }
          str += ": ";
          str += std::to_string(logprob_accessor[i][j]);
...
void RecWorkerImpl::LlmRecPureDevicePipeline::update_input_for_next_round(
  ...
  if (!input.input_params.llm_rec_pure_device_params
           .decode_positions_tensor_list.empty() &&
      current_step >= 0 &&
      current_step <
          static_cast<int32_t>(input.input_params.llm_rec_pure_device_params
                                   .decode_positions_tensor_list.size())) {
    input.positions = input.input_params.llm_rec_pure_device_params
                          .decode_positions_tensor_list[current_step];
  }
