...
// update_input_for_decode.";
#if defined(USE_CUDA)
  {
    ...
    auto dummy_query =
        torch::empty({batch_size * beam_size, num_heads, head_dim},
    ...
    input.input_params
        .decode_plan_info = kernel::cuda::generate_decode_plan_info(
...
// beam计数前缀和（未使用） out_sequence_ptr,    # [B, BEAM_SIZE, total_rounds]
// - 输出序列（写入）
#if defined(USE_NPU)
