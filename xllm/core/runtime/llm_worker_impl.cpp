...
void LLMWorkerImpl::update_for_next_step(
  ...
  beam_shared_kv_expanded = beam_shared_kv_expanded.unsqueeze(1).expand({-1, beam_size, -1});
  ...
  shared_kv_len_offsets = shared_kv_len_offsets.unsqueeze(1).expand({-1, beam_size, -1});
  // LOG(INFO) << "shared_kv_len_offsets: " << shared_kv_len_offsets;
  ...
  // [[0, 0, 0], [0, 0, 0], [0, 0, 0], ...]
  auto shared_batch_offsets = torch::zeros({batch_size, beam_size, shared_kv_len}, paged_options);
  ...
  auto batch_beam_lens_offsets = kv_cu_seq_lens.slice(0, 0, -1).unsqueeze(1).expand({-1, shared_kv_len});
  batch_beam_lens_offsets = batch_beam_lens_offsets.unsqueeze(1).expand({-1, beam_size, -1});
  ...
  auto unshared_paged_kv_indices = unshared_kv_indices.masked_select(unshared_mask);
  // LOG(INFO) << "unshared_paged_kv_indices: " << unshared_paged_kv_indices;
  auto shared_paged_kv_indptr_offsets = batch_shared_kv_lens.unsqueeze(1).expand({-1, beam_size});
  shared_paged_kv_indptr_offsets = shared_paged_kv_indptr_offsets.flatten().cumsum(0);
  shared_paged_kv_indptr_offsets = torch::cat({torch::zeros({1}, paged_options), shared_paged_kv_indptr_offsets}, 0);
  // LOG(INFO) << "shared_paged_kv_indptr_offsets: " << shared_paged_kv_indptr_offsets;
  ...
  auto unshared_paged_kv_indptr = torch::full({batch_size * beam_size}, unshared_kv_len, paged_options);
  unshared_paged_kv_indptr = unshared_paged_kv_indptr.flatten().cumsum(0);
  
  unshared_paged_kv_indptr = torch::cat({torch::zeros({1}, paged_options), unshared_paged_kv_indptr}, 0);
  ...
  // auto shared_paged_kv_last_page_len = torch::ones({batch_size}, paged_options);
