...
struct AttentionParams {
  ...
  void print() const {
    ...
    if (alibi_slope.has_value()) {
      LOG(INFO) << "alibi_slope shape: "
                << tensor_shape_str(alibi_slope.value());
    } else {
      LOG(INFO) << "alibi_slope: None";
    }

    if (q_quant_scale.has_value()) {
      LOG(INFO) << "q_quant_scale shape: "
                << tensor_shape_str(q_quant_scale.value());
    } else {
      LOG(INFO) << "q_quant_scale: None";
    }

    if (out_quant_scale.has_value()) {
      LOG(INFO) << "out_quant_scale shape: "
                << tensor_shape_str(out_quant_scale.value());
    } else {
      LOG(INFO) << "out_quant_scale: None";
    }

    if (block_table.has_value()) {
      LOG(INFO) << "block_table shape: "
                << tensor_shape_str(block_table.value());
    } else {
    ...
    LOG(INFO) << "paged_kv_indices shape: "
              << tensor_shape_str(paged_kv_indices);
    LOG(INFO) << "paged_kv_last_page_len shape: "
              << tensor_shape_str(paged_kv_last_page_len);
    LOG(INFO) << "float_workspace_buffer shape: "
              << tensor_shape_str(float_workspace_buffer);
    LOG(INFO) << "int_workspace_buffer shape: "
              << tensor_shape_str(int_workspace_buffer);
    LOG(INFO) << "page_locked_int_workspace_buffer shape: "
              << tensor_shape_str(page_locked_int_workspace_buffer);
    LOG(INFO) << "enable_cuda_graph: "
    ...
    if (q_cu_seq_lens.has_value()) {
      LOG(INFO) << "q_cu_seq_lens shape: "
                << tensor_shape_str(q_cu_seq_lens.value());
    } else {
      LOG(INFO) << "q_cu_seq_lens: None";
    }

    if (kv_cu_seq_lens.has_value()) {
      LOG(INFO) << "kv_cu_seq_lens shape: "
                << tensor_shape_str(kv_cu_seq_lens.value());
    } else {
    ...
    if (k_quant_scale.has_value()) {
      LOG(INFO) << "k_quant_scale shape: "
                << tensor_shape_str(k_quant_scale.value());
    } else {
      LOG(INFO) << "k_quant_scale: None";
    }

    if (v_quant_scale.has_value()) {
      LOG(INFO) << "v_quant_scale shape: "
                << tensor_shape_str(v_quant_scale.value());
    } else {
    ...
    if (k_cache_quant_scale.has_value()) {
      LOG(INFO) << "k_cache_quant_scale shape: "
                << tensor_shape_str(k_cache_quant_scale.value());
    } else {
      LOG(INFO) << "k_cache_quant_scale: None";
    }

    if (v_cache_quant_scale.has_value()) {
      LOG(INFO) << "v_cache_quant_scale shape: "
                << tensor_shape_str(v_cache_quant_scale.value());
    } else {
...
struct FusedLayerNormParams {
  ...
  void print() const {
    ...
    if (quant_scale.has_value()) {
      LOG(INFO) << "quant_scale shape: "
                << tensor_shape_str(quant_scale.value());
    } else {
      LOG(INFO) << "quant_scale: None";
    }

    if (residual_out.has_value()) {
      LOG(INFO) << "residual_out shape: "
                << tensor_shape_str(residual_out.value());
    } else {
      LOG(INFO) << "residual_out: None";
    }

    if (smooth_quant_scale.has_value()) {
      LOG(INFO) << "smooth_quant_scale shape: "
                << tensor_shape_str(smooth_quant_scale.value());
    } else {
    ...
    LOG(INFO) << "mode: " << mode;
    LOG(INFO) << "eps: " << eps;
    LOG(INFO) << "store_output_before_norm: "
              << (store_output_before_norm ? "true" : "false");
    LOG(INFO) << "store_output_after_norm: "
              << (store_output_after_norm ? "true" : "false");
