...
TEST_F(CacheSelctTest, CorrectnessTest) {
  ...
  // new_indices 的取值应覆盖 [0, beam_size * top_k)，否则 parent_beam 永远为 0
  torch::Tensor beam_index = torch::randint(
      0, beam_size * top_k, {batch_size * beam_size, 1}, int_opts);
  // Only need sorted values along dim=0; avoid std::tuple from sort().
  ...
  LOG(INFO) << "beam_after_sort: " << beam_after_sort;
  //   LOG(INFO) << "group_offset: " << group_offset;
  //   LOG(INFO) << "k_cache_intelligent: " << k_cache_intelligent[0];
  //   LOG(INFO) << "k_cache_normal: " << k_cache_normal[0];
  //   LOG(INFO) << "v_cache_intelligent: " << v_cache_intelligent[0];
  //   LOG(INFO) << "v_cache_normal: " << v_cache_normal[0];
  for (int64_t layer = 0; layer < layer_num; ++layer) {
