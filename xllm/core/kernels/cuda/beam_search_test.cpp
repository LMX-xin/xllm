...
TEST_F(BeamSearchTest, CorrectnessTest) {
  ...
  in_sequence_group = torch::randint(
  ...
  for (int32_t current_step = 0; current_step < total_rounds; current_step++) {
    ...
    if (current_step == 0) {
      top_tokens = torch::randint(0, 1000, {batch_size, 1, top_k}, int_options);
      top_logprobs = torch::randn({batch_size, 1, top_k}, fp_options);

    } else {
      top_tokens =
          torch::randint(0, 1000, {batch_size, beam_size, top_k}, int_options);
      top_logprobs = torch::randn({batch_size, beam_size, top_k}, fp_options);
    }
