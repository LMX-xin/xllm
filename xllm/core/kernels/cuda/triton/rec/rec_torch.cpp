...
torch::Tensor RecTorchKernel::xattention(
    torch::Tensor q,  // [batch_size, beam_size, num_heads, head_dim]
    torch::Tensor
        shared_k_cache,  // [num_shared_kv_seq_len, kv_heads, head_dim]
    torch::Tensor
        shared_v_cache,  // [num_shared_kv_seq_len, kv_heads, head_dim]
    torch::Tensor unshared_k_cache,  // [max_num_request, beam_size, kv_heads,
                                     // max_decode_step, head_dim]
    torch::Tensor unshared_v_cache,  // [max_num_request, beam_size, kv_heads,
                                     // max_decode_step, head_dim]
  ...
  torch::Tensor shared_o =
      torch::zeros_like(q);  // [batch_size, beam_size, num_heads, head_dim]
  torch::Tensor shared_m =
      torch::zeros({batch_size, beam_size, num_heads},
                   q.options());  // [batch_size, beam_size, num_heads]
...
void RecTorchKernel::prefill_reshape_and_cache(
...
void RecTorchKernel::decoder_reshape_and_cache(
...
void RecTorchKernel::shared(
    torch::Tensor q,  // [batch_size, beam_size, num_heads, head_dim]
    torch::Tensor
        shared_k_cache,  // [num_shared_kv_seq_len, kv_heads, head_dim].
                         // num_shared_kv_seq_len为最大长度padding
    torch::Tensor
        shared_v_cache,        // [num_shared_kv_seq_len, kv_heads, head_dim]
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    torch::Tensor target_Q =
        q.select(0, i);  // [beam_size, num_heads, head_dim]
    LOG(INFO) << "target_Q.shape: " << target_Q.sizes();
    int64_t group_size = num_heads / kv_heads;
    target_Q = target_Q.view(
        {beam_size,
         kv_heads,
         group_size,
         head_dim});  // [beam_size, kv_heads, group_size, head_dim]
    target_Q = target_Q.unsqueeze(
        -2);  // [beam_size, kv_heads, group_size, 1, head_dim]
    ...
    torch::Tensor target_batch_K = shared_k_cache.slice(
        0, begin, end);  // [cur_kv_seq_len, kv_heads, head_dim]
    torch::Tensor target_batch_V = shared_v_cache.slice(
        0, begin, end);  // [cur_kv_seq_len, kv_heads, head_dim]

    torch::Tensor acc = torch::zeros_like(
        target_Q);  // [beam_size, kv_heads, group_size, 1, head_dim]

    torch::Tensor m_i = torch::full(
        {beam_size, kv_heads, group_size, 1},
        -std::numeric_limits<float>::infinity(),
        target_Q.options());  // [beam_size, kv_heads, group_size, 1]
    torch::Tensor l_i = torch::zeros(
        {beam_size, kv_heads, group_size, 1},
        target_Q.options());  // [beam_size, kv_heads, group_size, 1]
    ...
    for (int64_t j = 0; j < cur_kv_seq_len; j += BLOCK_SIZE_N) {
      ...
      torch::Tensor target_batch_seq_K = target_batch_K.slice(
          0, seq_begin, seq_end);  // [seq_end - seq_begin, kv_heads, head_dim]
      torch::Tensor target_batch_seq_V = target_batch_V.slice(
          0, seq_begin, seq_end);  // [seq_end - seq_begin, kv_heads, head_dim]

      // 调整维度顺序以匹配矩阵乘法要求
      target_batch_seq_K = target_batch_seq_K.transpose(0, 1).unsqueeze(
          1);  // [kv_heads, 1, seq_end - seq_begin, head_dim]
      target_batch_seq_V = target_batch_seq_V.transpose(0, 1).unsqueeze(
          1);  // [kv_heads, 1, seq_end - seq_begin, head_dim]
      ...
      torch::Tensor qk =
          torch::matmul(target_Q, target_batch_seq_K.transpose(-2, -1)) * scale;

      torch::Tensor max_qk = std::get<0>(
          torch::max(qk, -1, true));  // [beam_size, kv_heads, group_size, 1, 1]
      max_qk = max_qk.squeeze(-1);    // [beam_size, kv_heads, group_size, 1]

      torch::Tensor m_j = max_qk;  // [beam_size, kv_heads, group_size, 1]

      torch::Tensor m_new =
          torch::maximum(m_i, m_j);  // [beam_size, kv_heads, group_size, 1]

      qk =
          qk -
          m_new.unsqueeze(
              -1);  // [beam_size, kv_heads, group_size, 1, seq_end - seq_begin]

      torch::Tensor p = torch::exp(
          qk);  // [beam_size, kv_heads, group_size, 1, seq_end - seq_begin]

      torch::Tensor l_j =
          torch::sum(p, -1);  // [beam_size, kv_heads, group_size, 1]
      ...
      torch::Tensor alpha =
          torch::exp(m_i - m_new);  // [beam_size, kv_heads, group_size, 1]
    ...
    acc = acc.view({beam_size,
                    group_size * kv_heads,
                    1,
                    head_dim});  // [beam_size, num_heads, 1, head_dim]
    l_i = l_i.view(
        {beam_size, group_size * kv_heads, 1});  // [beam_size, num_heads, 1]
    ...
    shared_l.select(0, batch_id)
        .copy_(l_i.squeeze(-1));  // [beam_size, num_heads]

    // shared_m [batch_size, beam_size, num_heads]
    m_i = m_i.view(
        {beam_size, group_size * kv_heads, 1});  // [beam_size, num_heads, 1]
    shared_m.select(0, batch_id)
        .copy_(m_i.squeeze(-1));  // [beam_size, num_heads]
  }
}
void RecTorchKernel::unshared(
    torch::Tensor q,  // [batch_size, beam_size, num_heads, head_dim]
    torch::Tensor unshared_k_cache,  // [max_num_request, beam_size, kv_heads,
                                     // max_decode_step, head_dim]
    torch::Tensor unshared_v_cache,  // [max_num_request, beam_size, kv_heads,
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    ...
    target_Q = target_Q.view(
        {beam_size,
         ...
         head_dim});  // [beam_size, kv_heads, group_size, head_dim]
    target_Q = target_Q.unsqueeze(
        -2);  // [beam_size, kv_heads, group_size, 1, head_dim] (添加seq维度)

    int64_t batch_id = i;
    int64_t block_id = block_table[batch_id][0].item().to<int64_t>();

    torch::Tensor target_request_K = unshared_k_cache.select(
        0, block_id);  // [beam_size, kv_heads, max_decode_step, head_dim]
    torch::Tensor target_request_V = unshared_v_cache.select(
        0, block_id);  // [beam_size, kv_heads, max_decode_step, head_dim]

    // 现在K和V已经是正确的维度顺序，只需要添加group维度
    target_request_K = target_request_K.unsqueeze(
        2);  // [beam_size, kv_heads, 1, max_decode_step, head_dim]
    target_request_V = target_request_V.unsqueeze(
        2);  // [beam_size, kv_heads, 1, max_decode_step, head_dim]

    // step可能是0和1，第0步是，需要读第1个kv，第1步时需要读2个kv
    torch::Tensor target_request_seq_K = target_request_K.slice(
        3, 0, step + 1);  // [beam_size, kv_heads, 1, step + 1, head_dim]
    torch::Tensor target_request_seq_V = target_request_V.slice(
        3, 0, step + 1);  // [beam_size, kv_heads, 1, step + 1, head_dim]

    torch::Tensor qk =
        torch::matmul(target_Q, target_request_seq_K.transpose(-2, -1)) *
        scale;  // [beam_size, kv_heads, group_size, 1, step + 1]

    torch::Tensor tmp_m = std::get<0>(
        torch::max(qk, -1));  // [beam_size, kv_heads, group_size, 1]

    torch::Tensor m = tmp_m.view({beam_size, num_heads, 1})
                          .squeeze(-1);  // [beam_size, num_heads]
    unshared_m.select(0, batch_id).copy_(m);

    torch::Tensor qk_shifted =
        qk -
        tmp_m.unsqueeze(-1);  // [beam_size, kv_heads, group_size, 1, step + 1]
    torch::Tensor qk_exp = torch::exp(
        qk_shifted);  // [beam_size, kv_heads, group_size, 1, step + 1]

    torch::Tensor tmp_l =
        torch::sum(qk_exp, -1);  // [beam_size, kv_heads, group_size, 1]
    torch::Tensor l = tmp_l.view({beam_size, num_heads, 1})
                          .squeeze(-1);  // [beam_size, num_heads]
    unshared_l.select(0, batch_id).copy_(l);

    torch::Tensor p =
        qk_exp /
        tmp_l.unsqueeze(-1);  // [beam_size, kv_heads, group_size, 1, step + 1]

    torch::Tensor output =
        torch::matmul(p, target_request_seq_V);  // [beam_size, kv_heads,
                                                 // group_size, 1, head_dim]
    output = output.view(
        {beam_size, num_heads, head_dim});  // [beam_size, num_heads, head_dim]
...
void RecTorchKernel::combine(torch::Tensor shared_o,
                             ...
                             torch::Tensor final_o) {
  // 步骤1: 反归一化，恢复累积状态
  torch::Tensor acc_shared =
      shared_o *
      shared_l.unsqueeze(-1);  // [batch_size, beam_size, num_heads, head_dim]
  torch::Tensor acc_unshared =
      unshared_o *
      unshared_l.unsqueeze(-1);  // [batch_size, beam_size, num_heads, head_dim]

  // 步骤2: 计算全局max（每个query位置独立）
  torch::Tensor m_global = torch::maximum(
      shared_m, unshared_m);  // [batch_size, beam_size, num_heads]

  // 步骤3: 重新缩放到全局max
  torch::Tensor alpha_shared =
      torch::exp(shared_m - m_global);  // [batch_size, beam_size, num_heads]
  torch::Tensor alpha_unshared =
      torch::exp(unshared_m - m_global);  // [batch_size, beam_size, num_heads]

  // 步骤4: 重新缩放累积值
  torch::Tensor acc_shared_rescaled =
      acc_shared * alpha_shared.unsqueeze(
                       -1);  // [batch_size, beam_size, num_heads, head_dim]
  torch::Tensor acc_unshared_rescaled =
      acc_unshared * alpha_unshared.unsqueeze(
                         -1);  // [batch_size, beam_size, num_heads, head_dim]
  torch::Tensor l_shared_rescaled =
      shared_l * alpha_shared;  // [batch_size, beam_size, num_heads]
  torch::Tensor l_unshared_rescaled =
      unshared_l * alpha_unshared;  // [batch_size, beam_size, num_heads]

  // 步骤5: 合并
  torch::Tensor acc_merged =
      acc_shared_rescaled +
      acc_unshared_rescaled;  // [batch_size, beam_size, num_heads, head_dim]
  torch::Tensor l_merged =
      l_shared_rescaled +
      l_unshared_rescaled;  // [batch_size, beam_size, num_heads]
...
void RecTorchKernel::cache_select(
  ...
  if (layer_num > 0) {
    ...
    // beam_index 是 new_indices，parent_beam = new_indices / top_k (top_k ==
    // beam_size)
    ...
    auto ori_k_cache = torch::zeros({layer_num,
                                     batch_size,
                                     beam_size,
                                     max_decode_step,
                                     kv_heads,
                                     head_dim});
    auto ori_v_cache = torch::zeros({layer_num,
                                     batch_size,
                                     beam_size,
                                     max_decode_step,
                                     kv_heads,
    ...
    for (int64_t layer = 0; layer < layer_num; ++layer) {
      ...
      for (int64_t b = 0; b < batch_size; ++b) {
        ...
        // 需要复制从 0 到 decode_step 的所有 step，因为新的 beam
        // 可能来自不同的旧 beam 例如：在 step 1 时，如果新的 beam 1 来自旧的
        // beam 0（而不是旧的 beam 1）， 那么新的 beam 1 的所有历史（step 0 和
        // step 1）都应该来自旧的 beam 0
