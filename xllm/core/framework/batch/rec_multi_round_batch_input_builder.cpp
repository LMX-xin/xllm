...
ForwardInput RecMultiRoundBatchInputBuilder::state_to_forward_input() {
  ...
  auto& rec_multi_round_state = rec_multi_round_state_;
  rec_multi_round_state.total_steps = get_rec_multi_round_decode_rounds();
  const int32_t beam_width = FLAGS_beam_width;
  const int32_t total_round = rec_multi_round_state.total_steps;
  ...
  if (!rec_multi_round_state.decode_selected_token_idxes.empty()) {
    CHECK_EQ(rec_multi_round_state.decode_sampling_params.size(),
             rec_multi_round_state.decode_selected_token_idxes.size());
    util::pad_2d_vector<int64_t>(
        rec_multi_round_state.decode_unique_token_ids_vec,
        /*pad_value=*/0);
    util::pad_2d_vector(rec_multi_round_state.decode_unique_token_counts_vec,
                        /*pad_value=*/0);

    forward_input.decoder_sampling_params.init(
        rec_multi_round_state.decode_sampling_params,
        rec_multi_round_state.decode_selected_token_idxes,
        rec_multi_round_state.decode_sample_idxes,
        rec_multi_round_state.decode_unique_token_ids_vec,
        rec_multi_round_state.decode_unique_token_counts_vec,
        rec_multi_round_state.decode_unique_token_lens_vec);
  }
  ...
  // Decode positions
  if (!rec_multi_round_state.decode_positions_vec.empty()) {
    decode_positions_vec = rec_multi_round_state.decode_positions_vec;
  }
