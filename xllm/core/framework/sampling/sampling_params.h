...
struct SamplingParameters {
 public:
  ...
  void print() const {
    ...
    if (top_p.defined()) {
      LOG(INFO) << "  top_p: " << top_p << " " << top_p.scalar_type();
    } else {
      LOG(INFO) << "  top_p: (undefined)";
    }

    if (top_k.defined()) {
      LOG(INFO) << "  top_k: " << top_k << " " << top_k.scalar_type();
    } else {
    ...
    // Scalar values
    LOG(INFO) << "  all_random_sample: "
              << (all_random_sample ? "true" : "false");
    LOG(INFO) << "  all_greedy_sample: "
              << (all_greedy_sample ? "true" : "false");
...
struct SampleOutput {
  ...
  void print() const {
    ...
    if (probs.defined()) {
      LOG(INFO) << "  probs: " << probs.sizes() << " " << probs.scalar_type();
    } else {
      LOG(INFO) << "  probs: (undefined)";
    }

    if (logprobs.defined()) {
      LOG(INFO) << "  logprobs: " << logprobs << " " << logprobs.scalar_type();
    } else {
