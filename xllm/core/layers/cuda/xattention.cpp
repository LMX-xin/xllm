...
std::tuple<torch::Tensor, std::optional<torch::Tensor>> XAttentionImpl::forward(
  ...
  if (attn_metadata.is_prefill) {
    ...
    if (FLAGS_enable_xattention_two_stage_decode) {
      ...
      SingleStageDecode(attn_metadata, query, output_tensor, key, batch_size);
    }
  }
  output_tensor = output_tensor.view({-1, num_heads_ * head_size_});
...
void XAttentionImpl::TwoStageDecode(const AttentionMetadata& attn_metadata,
                                    torch::Tensor& query,
  ...
  if (is_layer_0 && !attn_metadata.enable_cuda_graph) {
    ...
    bool need_init =
        ...
        attn_metadata.two_stage_decode_cache->cached_num_heads != num_heads_ ||
        attn_metadata.two_stage_decode_cache->cached_head_size != head_size_;
    ...
    if (need_init) {
      ...
      cache.shared_lse =
          torch::zeros({batch_size * beam_size, num_heads_, 1}, fp32_options);
      cache.shared_o = torch::zeros(
          {batch_size * beam_size, num_heads_, head_size_}, query.options());
      cache.unshared_lse =
          torch::zeros({total_beam, num_heads_, 1}, fp32_options);
      cache.unshared_o =
          torch::zeros({total_beam, num_heads_, head_size_}, query.options());
  ...
  auto shared_q = query.view({batch_size * beam_size, num_heads_, head_size_});
...
void XAttentionImpl::SingleStageDecode(const AttentionMetadata& attn_metadata,
                                       torch::Tensor& query,
                                       torch::Tensor& output_tensor,
                                       torch::Tensor& key,
                                       uint32_t batch_size) {
