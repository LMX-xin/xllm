...
std::tuple<torch::Tensor, std::optional<torch::Tensor>> XAttentionImpl::forward(
  ...
  if (attn_metadata.is_prefill) {
    ...
    if (FLAGS_enable_xattention_two_stage_decode) {
      forward_two_stage_decode(attn_metadata,
                               query,
                               key,
                               value,
                               output_tensor,
      ...
      forward_one_stage_decode(attn_metadata, query, key, value, output_tensor);
    }
  }
  output_tensor = output_tensor.view({-1, num_heads_ * head_size_});
...
void XAttentionImpl::forward_two_stage_decode(
    const AttentionMetadata& attn_metadata,
    torch::Tensor& query,
    torch::Tensor& key,
    torch::Tensor& value,
...
void XAttentionImpl::forward_one_stage_decode(
    const AttentionMetadata& attn_metadata,
    torch::Tensor& query,
    torch::Tensor& key,
    torch::Tensor& value,
